...
def murcko_scaffold(smiles, include_chirality=False, unique_acyclic=True):
    """Return the Bemis-Murcko scaffold of a SMILES string.

    A molecule with no ring system has no Murcko scaffold and RDKit returns
    an empty string for it.  On datasets such as ESOL and FreeSolv, which
    are largely small acyclic molecules, that puts a large fraction of the
    data into a single group, and since a group is never split across folds
    the result is wildly uneven folds.

    With ``unique_acyclic`` the molecule's own canonical SMILES is used
    instead of the empty scaffold, so each acyclic molecule forms its own
    group.  Nothing leaks by doing this: the point of a scaffold split is
    that a core seen in training must not reappear in test, and a molecule
    with no ring system has no core to share.
    """
    try:
    ...
    if not unique_acyclic:
        return ""

    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return f"unparsed:{smiles}"
    return "acyclic:" + Chem.MolToSmiles(mol)


def scaffold_groups(smiles_list, include_chirality=False, unique_acyclic=True):
